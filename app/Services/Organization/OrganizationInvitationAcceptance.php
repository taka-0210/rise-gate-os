...
class OrganizationInvitationAcceptance
{
    ...
    public function prepare(Request $request, User $user): OrganizationUser
    {
        ...
        $organization = Organization::query()->findOrFail($snapshot->organization_id);

        return $this->productAdmission->prepareExistingOrganization(
            $user,
            $organization,
            ProductOrganizationAdmission::ENTRY_STAFF_PREPARE,
            function () use ($snapshot, $user): OrganizationUser {
    ...
    public function accept(Request $request, User $user): OrganizationInvitation
    {
        ...
        $organization = Organization::query()->findOrFail($snapshot->organization_id);
        $accepted = $this->productAdmission->admitExistingOrganization(
            $user,
            $organization,
            ProductOrganizationAdmission::ENTRY_STAFF_ACCEPT,
            function () use ($snapshot, $user): OrganizationInvitation {
                return DB::transaction(function () use ($snapshot, $user): OrganizationInvitation {
