            ...
            <tbody>
                @foreach($rows as $row)
                    <tr>
                        ...
                        <td><span class="capacity-type {{ $row['type'] === '実績' ? 'actual' : 'plan' }}">{{ $row['type'] }}</span></td>
                        ...
                        <td>{{ $row['coverage_ratio'] === null ? '—' : number_format($row['coverage_ratio'], 2).'倍' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
