...
@section('content')
<div class="page-header"><div><div class="meta"><a href="{{ route('company-finance.pl.index') }}">P/L</a> / BULK INPUT</div><h1>表を貼り付けて一括入力</h1><p>Excelやスプレッドシートから、11列をそのまま貼り付けられます。従来の10列形式にも対応しています。</p></div></div>
<div class="card">
<div class="bulk-columns" aria-label="貼り付ける列の順番">
@foreach(['期','年度','売上高','売上原価','販管費','営業外収益','営業外費用','内：支払利息','特別利益','特別損失','法人税等'] as $label)
    <div><b>{{ $loop->iteration }}</b><span>{{ $label }}</span></div>
@endforeach
</div>
<form method="POST" action="{{ route('company-finance.pl.bulk.preview') }}">@csrf
@if($errors->any())<div class="alert error">{{ $errors->first() }}</div>@endif
<textarea name="bulk_text" rows="12" style="width:100%;font-family:monospace" placeholder="21	2024	100000000	60000000	30000000	0	2000000	1500000	0	0	3000000">{{ old('bulk_text') }}</textarea>
