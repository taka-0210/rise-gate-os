...
<section class="stack">
    ...
    <form method="POST" action="{{ route('workspace-business-profile.update') }}" enctype="multipart/form-data" class="stack">@csrf @method('PUT')
        <section class="card stack"><div><h2>会社情報</h2><p class="meta">帳票に表示する正式な発行元情報です。</p></div><div class="profile-grid">
            <div class="field"><label>法人名・正式名称 <span class="required-mark">必須</span></label><input name="legal_name" value="{{ old('legal_name',$profile?->legal_name) }}" required @disabled(!$canManage)></div>
        ...
        <section class="card stack"><div><h2>主振込口座</h2><p class="meta">請求書へ表示する標準の振込先です。口座を登録する場合は「必須」の4項目を入力してください。口座は後から登録しても構いません。</p></div><div class="profile-grid">
            <div class="field"><label>金融機関名 <span class="required-mark">口座登録時は必須</span></label><input name="bank_name" value="{{ old('bank_name',$bankAccount?->bank_name) }}" @disabled(!$canManage)></div>
            ...
            <div class="field"><label>口座種別 <span class="required-mark">口座登録時は必須</span></label><select name="account_type" @disabled(!$canManage)>@foreach($accountTypes as $value=>$label)<option value="{{ $value }}" @selected(old('account_type',$bankAccount?->account_type??'ordinary')===$value)>{{ $label }}</option>@endforeach</select></div>
            <div class="field"><label>口座番号 <span class="required-mark">口座登録時は必須</span></label><input name="account_number" value="{{ old('account_number',$bankAccount?->account_number) }}" @disabled(!$canManage)></div>
            <div class="field span-2"><label>口座名義 <span class="required-mark">口座登録時は必須</span></label><input name="account_holder" value="{{ old('account_holder',$bankAccount?->account_holder) }}" @disabled(!$canManage)></div>
        </div></section>
...
<style>.profile-grid,.media-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.span-2{grid-column:1/-1}.required-mark{display:inline-flex;margin-left:4px;padding:2px 6px;border-radius:999px;background:#fff0ed;color:#a84e3c;font-size:10px;font-weight:800;vertical-align:2px}.profile-media{display:block;max-width:260px;max-height:120px;margin:8px 0;padding:8px;border:1px solid var(--line);border-radius:8px;background:#fff;object-fit:contain}.seal-preview{width:110px;height:110px}.remove{display:flex;align-items:center;gap:7px;margin:7px 0;color:var(--muted);font-size:12px}.remove input{width:auto}@media(max-width:700px){.profile-grid,.media-grid{grid-template-columns:1fr}.span-2{grid-column:auto}}</style>
@endsection
