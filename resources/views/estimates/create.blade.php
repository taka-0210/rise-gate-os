...
@section('content')
@php
    $effortMode = request('source') === 'effort';
    $dailyRate = max(0, (int) old('daily_rate', 50000));
    $row = fn (string $type, $id, string $title, int $level, $effort = null) => [
        'type' => $type, 'id' => $id, 'title' => $title, 'level' => $level,
        'effort' => $effort, 'selected' => $effortMode && $type === 'improvement' && $effort !== null,
        'quantity' => $effortMode && $type === 'improvement' && $effort !== null ? (float) $effort : 1,
        'unit' => $effortMode && $type === 'improvement' && $effort !== null ? '人日' : '式',
        'unit_price' => $effortMode && $type === 'improvement' && $effort !== null ? $dailyRate : 0,
    ];
    $rows = collect([$row('manual', null, $project->name.' 一式', 0)]);
    foreach ($project->roadmaps as $roadmap) {
        $rows->push($row('roadmap', $roadmap->id, $roadmap->title, 0));
        foreach ($roadmap->improvements as $improvement) {
            $rows->push($row('improvement', $improvement->id, $improvement->title, 1, $improvement->planned_effort_days));
            foreach ($improvement->tasks as $task) $rows->push($row('task', $task->id, $task->title, 2));
        }
    }
    foreach ($project->improvements->whereNull('roadmap_id') as $improvement) {
        $rows->push($row('improvement', $improvement->id, $improvement->title, 0, $improvement->planned_effort_days));
        foreach ($improvement->tasks as $task) $rows->push($row('task', $task->id, $task->title, 1));
    }
    $effortRows = $rows->where('type', 'improvement')->whereNotNull('effort');
    $unsetEffortCount = $rows->where('type', 'improvement')->whereNull('effort')->count();
    $totalEffort = (float) $effortRows->sum('effort');
    $formatEffort = fn (float $value) => rtrim(rtrim(number_format($value, 2, '.', ''), '0'), '.');
@endphp
<section class="stack">
    <div><div class="meta">{{ $project->client->name }} / {{ $project->name }}</div><h1>{{ $effortMode ? '工数から見積書を作成' : '見積書を作成' }}</h1><p>見積へ載せる項目を選び、数量と単価を入力します。保存後はProject側を変更しても見積内容は変わりません。</p></div>
    ...
    <form id="estimate-form" method="POST" action="{{ route('projects.estimates.store',$project) }}" class="stack">@csrf
        ...
        @if($effortMode)
            <section class="effort-estimate-setup">
                <div><span>取り込み対象</span><strong>{{ $formatEffort($totalEffort) }}人日</strong><small>工数設定済み {{ $effortRows->count() }}件 @if($unsetEffortCount)・未設定 {{ $unsetEffortCount }}件は対象外@endif</small></div>
                <div class="field"><label for="daily-rate">基準人日単価（円）</label><input id="daily-rate" name="daily_rate" type="number" min="0" step="1000" value="{{ $dailyRate }}"></div>
                <button class="button secondary" type="button" id="apply-daily-rate">全取り組みに単価を適用</button>
            </section>
        @else
            <div class="panel effort-mode-link"><span>取り組みに工数を設定済みなら、数量を自動入力できます。</span><a class="button secondary" href="{{ route('projects.estimates.create', ['project' => $project, 'source' => 'effort']) }}">工数から作成</a></div>
        @endif
        <section class="card stack"><div><h2>見積明細</h2><p class="meta">{{ $effortMode ? '工数設定済みの取り組みを選択し、数量を人日で取り込みました。不要な項目は選択を外せます。' : '一式を選ぶと、ほかの選択項目は金額に加算せず「作業範囲」として見積書へ記載します。' }}</p></div><div class="scope-actions"><button class="button secondary" type="button" id="select-all-scope">作業範囲をすべて選択</button><button class="button secondary" type="button" id="clear-all-scope">すべて解除</button><span id="package-mode-note">ロードマップや取り組みを選ぶと、配下もまとめて選択できます。</span></div><div class="estimate-items">
            @forelse($rows as $index=>$row)<div class="estimate-item level-{{ $row['level'] }} @if($row['type']==='manual') package-item @endif" data-type="{{ $row['type'] }}" data-level="{{ $row['level'] }}" @if($row['effort'] !== null) data-effort-days="{{ (float) $row['effort'] }}" @endif><input type="hidden" name="items[{{ $index }}][selected]" value="0"><label class="select-item"><input class="item-check" type="checkbox" name="items[{{ $index }}][selected]" value="1" @checked((bool) old("items.$index.selected", $row['selected']))><span class="type type-{{ $row['type'] }}">{{ ['manual'=>'一式見積','roadmap'=>'ロードマップ','improvement'=>'取り組み','task'=>'タスク'][$row['type']] }}</span></label><input type="hidden" name="items[{{ $index }}][source_type]" value="{{ $row['type'] }}"><input type="hidden" name="items[{{ $index }}][source_id]" value="{{ $row['id'] }}"><div class="field description"><label>{{ $row['type']==='manual' ? '明細名' : '作業内容' }}</label><input name="items[{{ $index }}][description]" value="{{ old("items.$index.description",$row['title']) }}" required></div><div class="field small price-field"><label>数量</label><input type="number" step="0.001" min="0.001" name="items[{{ $index }}][quantity]" value="{{ old("items.$index.quantity", $row['quantity']) }}" required></div><div class="field small price-field"><label>単位</label><input name="items[{{ $index }}][unit]" value="{{ old("items.$index.unit", $row['unit']) }}" required></div><div class="field price price-field"><label>単価（円）</label><input type="number" min="0" name="items[{{ $index }}][unit_price]" value="{{ old("items.$index.unit_price", $row['unit_price']) }}" required></div><div class="field tax price-field"><label>税率</label><select name="items[{{ $index }}][tax_rate]"><option value="10">10%</option><option value="8">8%</option><option value="0">0%</option></select></div><div class="scope-only-label">作業範囲として記載</div></div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('apply-daily-rate')?.addEventListener('click', () => {
        const rate = Math.max(0, Number(document.getElementById('daily-rate')?.value || 0));
        document.querySelectorAll('[data-effort-days]').forEach(row => {
            row.querySelector('[name$="[unit_price]"]').value = String(rate);
        });
        calculate();
    });
