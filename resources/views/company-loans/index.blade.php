    ...
    @else
        ...
        <form class="card loan-table-wrap" method="POST" action="{{ route('company-loans.confirm-drafts') }}">
            ...
            @foreach($loans as $loan)<tr class="{{ $loan->loan_status==='completed'?'is-completed':'' }}">
                ...
                <td><span class="status status--{{ $loan->loan_status }}">{{ ['active'=>'借入中','completed'=>'完済','planned'=>'実行予定'][$loan->loan_status] }}</span>@if($loan->loan_status==='completed')<br><small>{{ $loan->completed_on?->format('Y.m.d') ?: '完済日未登録' }}</small>@endif<br><small>{{ $loan->record_status==='confirmed'?'確定':'下書き' }}</small></td>
