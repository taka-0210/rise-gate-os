...
@section('content')
    <section class="panel stack">
        ...
        <form class="stack" method="POST" action="{{ route('projects.tasks.update', [$project, $task]) }}">
            ...
            <div class="field">
                ...
                <select id="improvement_id" name="improvement_id" required>
                    @foreach ($initiatives as $initiative)
                        <option value="{{ $initiative->id }}" @selected((string) old('improvement_id', $task->improvement_id) === (string) $initiative->id)>{{ $initiative->roadmap?->title }} / {{ $initiative->title }}</option>
                    @endforeach
                </select>
            ...
            @unless(!$project->start_date && $project->duration_days)
        ...
        <form method="POST" action="{{ route('projects.tasks.destroy',[$project,$task]) }}" onsubmit="return confirm('このタスクを削除しますか？');">
            @csrf @method('DELETE')
            <button type="submit" class="secondary">タスクを削除</button>
        </form>
    </section>
@endsection
