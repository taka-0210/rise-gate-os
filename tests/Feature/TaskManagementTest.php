...
class TaskManagementTest extends TestCase
{
    ...
    public function test_project_has_an_optional_three_pane_workspace(): void
    {
        ...
        $this->actingAs($owner)
            ...
            ->assertSee('ブラウザ')
            ->assertSee('管理画面のプレビュー')
            ->assertDontSee('作業ペイン内では閲覧確認のみ行えます。')
            ->assertSee('管理画面を別タブで開く')
            ->assertSee('data-browser-external-notice', false)
            ->assertSee('showBrowserPreview(workspaceTab.dataset.tabUrl, workspaceTab.dataset.tabKey)', false)
            ...
            ->assertDontSee('フォルダを開いて成果物を確認する')
            ->assertSee('現行表示へ戻る')
            ...
            ->assertSee('data-hierarchy-level="roadmap"', false)
            ->assertSee('data-hierarchy-level="improvement"', false)
            ->assertSee('data-hierarchy-level="task"', false)
            ->assertSee('setHierarchyLevel', false)
            ->assertSee('data-document="task-'.$task->id.'"', false)
            ->assertSee('data-document-panel="task-'.$task->id.'"', false)
            ->assertSee('data-pane-resizer="explorer"', false)
